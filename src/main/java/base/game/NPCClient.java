...
public class NPCClient implements IClientMessageHandler {
    ...
    public NPCClient(Configuration config, String playerName) {
        this.config = config;
        this.comClient = new ComClient(config, this, playerName);
        this.playerName = playerName;
        this.assist = new Assist();
        log = new Logger(playerName, 0, true, 0);
    }

    @Override
    public void handleInput(Message message) {
        System.out.println(this.playerName + ":" + message.getCommand());
    ...
    private void handleSelectGameDoko() {
        ...
        MessageGameSelected.GAMES game = NORMAL;
        if (hand.stream().filter(card -> card.kind.equals(Statics.KOENIG)).count() > 4) {
            send(new MessageGameSelected(players.indexOf(playerName), KOENIGE));
        }
        double risk = 4.5;
        List<MessageGameSelected.GAMES> gameList = new ArrayList<>();
        for (MessageGameSelected.GAMES g : MessageGameSelected.GAMES.values()) {
            if(g.value>0 && g.value<9){
                gameList.add(g);
            }
        }
        for (MessageGameSelected.GAMES g : gameList) {
            assist.setGame(g);
            double calcRisk = assist.playSolo();
            if (calcRisk < risk) {
                game = g;
                risk = calcRisk;
            }
        ...
        if (game == NORMAL) {
            if (hand.stream().filter(card -> card.trump).count() < 4) {
                game = ARMUT;
            } else if (hand.stream().filter(card -> card.suit.equals(Statics.KREUZ) && card.kind.equals(Statics.DAME)).count() > 1) {
                game = HOCHZEIT;
            }
        }
        send(new MessageGameSelected(players.indexOf(playerName), game));
    }
